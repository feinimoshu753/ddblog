---
title: Vue源码解析（五）
date: 2017-09-06 23:03:45
tags: 前端
---
接下来看看renderMixin方法，这个方法在Vue的原型上增加了`$nextTick`、`_render`以及`_o、_n、_s`等方法。
...
```
`$nextTick`方法传入一个函数，同时调用nextTick方法，传入的fn是dom刷新之后的回调。
```
...
```
vue的原型上添加了`_o、_n、_s` 等14个内部方法，源码的注释说明是render函数的一些帮助函数，都放在了vue的原型上。这个地方不是很理解的是为什么要采用单字母来命名方法，有什么特别的意义？